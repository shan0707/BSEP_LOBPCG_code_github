function [X, max_res, eig_M, out]= spopt(X, fun, opts, Max_time, test, varargin)
%%-------------------------------------------------------------------------
...
if metric ~= 1
    retr = 2; metricname = 'Euclidean'; %   cannot be applied to Euclidean metric
else
...
for itr = 1 : opts.mxitr
    ...
    %% check convergence
    if FDiff < ftol && mod(itr, 50) == 0
        MX = M*X;
        XMX = X'*MX;  XMX = 0.5*(XMX + XMX');
        [Sr, eig_M] = williasondiag(XMX);
        Xmin = X*Sr;
        QXQ = test.Qn*Xmin*test.Qk';
        max_res = compute_Res(test.Omega, test.Omega_norm, QXQ, eig_M, n, k, opts.tol);
        %MXmin = M*Xmin;
        %JXmin = [Xmin(n+1:2*n,:);-Xmin(1:n,:)];
        %R = MXmin - JXmin*[zeros(k,k) -diag(eig_M); 
        %                   diag(eig_M) zeros(k,k)];
        %for j = 1 : k
        %    R_norm(j) = norm([R(:, j), R(:, j + k)])/...
        %        ((M_norm + abs(eig_M(j)))*norm([X(:, j), X(:, j + k)]));
        %end
        %max_res = max(R_norm);
        if max_res < opts.tol
            break;
